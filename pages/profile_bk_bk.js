...
Profile.getInitialProps = async ctx =>{
  ...
  const auth_headers = await response.headers
  ...
  if(resstatus===200){
    ...
    const response_1 = await fetch(apiUrl,options)
    ...
    if(resstatus_1===401){
      const response_2 = await fetch(apiUrl,options)
      const auth_headers_2 = await response_2.headers
      const resstatus_2 = await response_2.status;
    
      console.log( auth_headers_2)
      console.log(`resend 2times`+ resstatus_2)
    }
  }
  else{
